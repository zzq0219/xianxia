...
const CharacterDetail: React.FC<CharacterDetailProps> = ({ card, onClose, playerProfile, setPlayerProfile, onViewPet }) => {
    ...
    const handleEquip = (slot: keyof CharacterCard['equipment'], item: Equipment) => {
        updateCard((cardToUpdate, profile) => {
            ...
            // After equipping, fully heal the character
    ...
    const handleUnequip = (slot: keyof CharacterCard['equipment']) => {
        updateCard((cardToUpdate, profile) => {
            ...
            if (oldItem) {
                ...
                // After unequipping, fully heal the character
                const newTotalAttributes = calculateTotalAttributes(cardToUpdate);
                cardToUpdate.baseAttributes.hp = newTotalAttributes.maxHp;
                cardToUpdate.baseAttributes.mp = newTotalAttributes.maxMp;
            }
        ...
        <>
            ...
            >
                ...
                >
                            ...
                            <div className="bg-black/20 p-4 rounded-lg border border-stone-700/50">
                                ...
                                <div className="space-y-3">
                                    {displayedCard.skills.map((skill, index) => (
                                        <div key={index} className={`relative w-full text-left bg-stone-700/50 p-3 rounded-md border-2 ${!skill ? 'border-dashed border-stone-600' : getRarityBorderColor(skill?.rarity)} ${skill ? 'cursor-pointer hover:bg-stone-600/70' : ''} transition-colors`} onClick={() => {
                                            if (skill) {
                    ...
                    <div className="bg-black/20 p-4 rounded-lg border border-stone-700/50">
                        <h4 className="font-semibold text-amber-400 mb-3 text-lg">兽宠</h4>
                        <div className="relative">
                            <button
                                ...
                                className={`bg-stone-700/50 p-3 rounded-md w-full text-left hover:bg-stone-600/70 transition-colors h-full border-2 ${getRarityBorderColor(displayedCard.pet?.rarity)}`}
                            >
                                <p className="text-sm text-gray-400">出战兽宠</p>
                                <p className={`font-bold truncate ${getRarityTextColor(displayedCard.pet?.rarity)}`}>{displayedCard.pet?.name || '未携带'}</p>
                            </button>
                            {displayedCard.pet && (
                                <button onClick={(e) => { e.stopPropagation(); handleUnequipPet(); }} className="absolute -top-1.5 -right-1.5 bg-red-600 text-white w-5 h-5 rounded-full text-xs font-bold flex items-center justify-center hover:bg-red-500 transition-colors z-10 border border-stone-900" title="卸下">×</button>
                            )}
                        </div>
                    </div>
                </div>
