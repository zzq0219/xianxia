...
const CombatActions: React.FC<CombatActionsProps> = ({ activeCard, onAction, onClose }) => {
    ...
    return (
        ...
        >
            ...
            >
                <div className="grid grid-cols-3 md:grid-cols-5 gap-4" style={{ gridAutoRows: '1fr' }}>
                    <ActionButton
                        title="普通攻击"
                    ...
                    <ActionButton title="防御" description="(未实装)" disabled />
                    <ActionButton title="道具" description="(未实装)" disabled />
                    <ActionButton title="逃跑" description="(未实装)" disabled />
                </div>
            </div>
        </div>
    );
