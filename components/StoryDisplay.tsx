...
const CornerDecoration = ({ className }: { className?: string }) => (
  <svg className={`absolute w-16 h-16 text-xianxia-gold-500/40 pointer-events-none ${className}`} viewBox="0 0 100 100" fill="currentColor">
...
const StoryDisplay: React.FC<StoryDisplayProps> = ({ story, storyEndRef }) => {
  return (
    <div className="relative w-full max-w-4xl h-full flex flex-col group">
      {/* 背景容器层 - 包含背景、边框和基础装饰 */}
      <div className="absolute inset-0 bg-stone-950/90 rounded-xl border-2 border-xianxia-gold-700/30 shadow-2xl backdrop-blur-md overflow-hidden">
        {/* 内层装饰边框 */}
        <div className="absolute inset-1 border border-xianxia-gold-500/20 rounded-lg pointer-events-none" />

        {/* 角落装饰 */}
        <CornerDecoration className="top-0 left-0" />
        <CornerDecoration className="top-0 right-0 transform rotate-90" />
        <CornerDecoration className="bottom-0 right-0 transform rotate-180" />
        <CornerDecoration className="bottom-0 left-0 transform -rotate-90" />
      ...
      <div className="relative z-10 w-full h-full overflow-y-auto scrollbar-xianxia p-4 sm:p-8">
        <div className="prose max-w-none prose-p:text-amber-50 prose-p:leading-loose prose-p:tracking-widest prose-headings:text-xianxia-gold-400 prose-headings:font-xianxia prose-strong:text-xianxia-gold-300 font-xianxia whitespace-pre-wrap animate-fade-in text-lg sm:text-xl md:text-2xl text-shadow-sm drop-shadow-md selection:bg-xianxia-gold-900/50 selection:text-xianxia-gold-100">
          {story}
        </div>
        <div ref={storyEndRef} />
      </div>
