...
const PartyMember: React.FC<{char: CharacterCard}> = ({char}) => {
    ...
    return (
        <div className="bg-stone-700/50 p-3 rounded-lg backdrop-blur-sm">
...
const InfoCard: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
    <div className="bg-black/20 p-4 rounded-lg border border-stone-700/50">
        <h3 className="font-semibold text-amber-400 mb-3 text-lg">{title}</h3>
        {children}
    </div>
);
...
const PersonalInfoPanel: React.FC<PersonalInfoPanelProps> = ({ playerProfile, isOpen, onClose, hasPendingChallenge, onBattleClick, onRelationshipsClick, onReputationClick }) => {
  return (
    <>
      ...
      {/* Panel */}
      <div
        className={`fixed top-0 left-0 h-full w-full max-w-md bg-stone-900/90 backdrop-blur-lg border-r border-yellow-400/20 shadow-2xl z-40 transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        <div className="p-4 flex justify-between items-center border-b border-yellow-400/20 bg-stone-900/50">
          <div className="flex items-center gap-4">
            <h2 className="text-xl font-bold text-white font-serif">个人信息</h2>
            <button
              onClick={onBattleClick}
              className={`relative px-4 py-1.5 text-sm font-semibold rounded-md transition-all duration-300 border ${
                hasPendingChallenge 
                ? 'bg-red-800/80 border-red-500 text-white animate-pulse-fast shadow-[0_0_15px_rgba(239,68,68,0.6)]' 
        ...
        <div className="p-4 overflow-y-auto h-[calc(100%-65px)] space-y-4 font-serif">
