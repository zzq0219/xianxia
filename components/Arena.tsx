...
import { CharacterCard, Leaderboards, PlayerProfile } from '../types';
...
const TabButton: React.FC<{ label: string; isActive: boolean; onClick: () => void, isSubTab?: boolean }> = ({ label, isActive, onClick, isSubTab }) => (
    <button
        onClick={onClick}
        className={`px-4 transition-colors duration-200 font-semibold ${
            isSubTab
...
const Arena: React.FC<ArenaProps> = ({ playerProfile, leaderboards, onMatchFound }) => {
